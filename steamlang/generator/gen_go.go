...
const (
	...
	pkgGolangProto = "google.golang.org/protobuf/proto"
)

// GenGo implements a Go code generator. The zero value for GenGo is an empty generator ready to
// use.
type GenGo struct {
	...
	// Imports are packages that need to be imported by the generated code.
...
func (g *GenGo) Generate(w io.Writer, root *parser.Node) error {
	...
	if _, err := io.Copy(&body, &g.buf); err != nil {
		return err
	}
	...
	// copy body to generator buffer
	if _, err := io.Copy(&g.buf, &body); err != nil {
		return err
	}
	...
	if _, err := io.Copy(w, &g.buf); err != nil {
		return err
	}
...
func (g *GenGo) validateAndFormat() error {
	...
	if err != nil {
		if errl := (scanner.ErrorList{}); errors.As(err, &errl) {
			err = NewSyntaxError(string(src), errl, 5)
		}

		return fmt.Errorf("invalid generated source: %w", err)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("generated source could not be formatted: %w", err)
	}

	g.buf.Reset()

	if _, err := g.buf.Write(fmtSrc); err != nil {
		return err
	}
...
func (g *GenGo) writeHeader() error {
	g.sn("// Code generated by steamlang-gen-go. DO NOT EDIT.")
	// package {{Package}}
